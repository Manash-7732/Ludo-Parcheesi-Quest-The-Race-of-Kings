class Dice {
  ...
  roll() {
    ...
    if (this.type == 1) {
      ...
      this.yctns.splice(0, this.yctns.length);
      if (this.count != 6) {
        if (this.numOfPlayers == 2) this.type--;
        else this.type++;
      }
      console.log("Green");
    } else if (this.type == 3) {
...
function changeNumOfPlayers(numOfPlayers) {
  console.log("Number of players changed to:", numOfPlayers);
...
function resetBoard() {
  ...
  roll = new Dice((numOfPlayers = noOfPlayers));
  red = new Red();
  green = new Green();
  yellow = new Yellow();
  blue = new Blue();

  msg.innerHTML = "Board has been reset!";
}

var roll = new Dice((numOfPlayers = noOfPlayers));
var red = new Red();
var green = new Green();
